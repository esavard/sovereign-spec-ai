...
# --- 1. Path Detection (Sidecar Mode) ---
# Physical location of the script (sovereign-spec-ai/)
...
def init_env():
    ...
    # Aider protection (create .aiderignore at the project root)
    ...
    if not os.path.exists(aiderignore_path):
        with open(aiderignore_path, "w") as f:
            f.write(f"/{tool_dir}/\n")
            f.write("architecture*/\n")  # Optional: prevent Aider from modifying the blueprint
...
def run_agent(command, env_updates=None):
    ...
    # Force Python and Aider to flush logs in real time
    custom_env["PYTHONUNBUFFERED"] = "1"
    custom_env["AIDER_NO_COLORS"] = "1"  # Avoids display glitches caused by terminal escape codes
    ...
    try:
        process = subprocess.Popen(
            ...
            text=True,
            env=custom_env,
            cwd=PROJECT_ROOT  # Ensure Aider runs in the project root, not inside the tool directory
        )
...
def trigger_dev_swarm(task_filename):
    ...
    if result == 0:
        ...
        st.error(f"Agent failed (exit code {result}). Process paused — check the log above.")
        return False
...
with st.sidebar:
        ...
        if selected_bp:
            ...
            if st.button("Generate Backlog"):
                ...
                if result == 0:
                    ...
                    st.error(f"Error (code {result}). Check the log above.")
    ...
    if "editing" in st.session_state:
        ...
        st.divider()
        st.subheader(f"Edit: {os.path.basename(file_path)}")
